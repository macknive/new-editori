<template>
  <aside
    class="approval-root prevent-default-component-style"
...
<script>
export default {
  props: ['assignee', 'currentStepId', 'deliverable', 'viewer'],
  methods: {
    approve() {
    ...
    goToStep(stepIndex) {
      ...
      const step = this.deliverable.workflow.steps.find(
  ...
  computed: {
    ...
    currentStepIndex() {
      return this.workflowData.findIndex(
        instance => instance.step.id == this.currentStepId
      )
    },
    nextStepIndex() {
      if (this.currentStepIndex < 0) {
